...
# ==========================================
# 2. INPUT: DEEZER GATEWAY (v9.0 Headers & Page Strategy)
# ==========================================
...
class DeezerGatewaySource(InputSource):
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            ...
            'X-Requested-With': 'XMLHttpRequest', # <--- THE FIX
    ...
    def authenticate(self):
        ...
        # 2. Manual
        if not self.arl:
            self.arl = input("    Please paste the value of the 'arl' cookie: ").strip()

        if not self.arl: raise Exception("No ARL provided.")
    ...
    def _refresh_token(self):
        ...
        if not self.loved_playlist_id:
            self.loved_playlist_id = data['USER'].get('LOVEDTRACKS_ID') or data['USER'].get('loved_tracks_playlist')
    ...
    def _raw_call(self, method, body_dict, check_token=True):
        ...
        clean_body = {k: str(v) if isinstance(v, (int, float)) else v for k, v in body_dict.items()}
        ...
        try:
            ...
            if 'error' in response_json:
                ...
                if isinstance(error_list, dict):
                    ...
                    if check_token and (err_type == 'VALID_TOKEN_REQUIRED' or 'token' in str(error_list).lower()):
    ...
    def fetch_data(self) -> dict:
        ...
        if self.loved_playlist_id:
            ...
            fav_tracks = self._fetch_full_playlist(self.loved_playlist_id, "Loved Tracks")
        
        # Fallback if playlist ID method returned 0 (and we expected more)
        if len(fav_tracks) == 0:
            print("   [!] Playlist strategy returned 0. Trying 'portal.getUserTracks' fallback...")
            start = 0
            while True:
                data = self._raw_call('portal.getUserTracks', {'user_id': self.user_id, 'start': start, 'nb': 50})
                items = data.get('data', [])
                if not items: break
        ...
        fav_albums = []
        albums_page = self._raw_call("deezer.pageProfile", {'tab': 'albums', 'user_id': self.user_id})
        ...
        fav_artists = []
        artists_page = self._raw_call("deezer.pageProfile", {'tab': 'artists', 'user_id': self.user_id})
        ...
        pl_page = self._raw_call("deezer.pageProfile", {'tab': 'playlists', 'user_id': self.user_id})
        raw_playlists = []
        if 'TAB' in pl_page and 'playlists' in pl_page['TAB']:
            raw_playlists = pl_page['TAB']['playlists'].get('data', [])
            
        for pl in raw_playlists:
            creator_id = pl.get('PARENT_USER_ID') or pl.get('CREATOR', {}).get('USER_ID')
            pl_id = pl.get('PLAYLIST_ID') or pl.get('id')
            title = pl.get('TITLE') or pl.get('title')
            
            if str(creator_id) == str(self.user_id) and str(pl_id) != str(self.loved_playlist_id):
        ...
        return {
            ...
            "user_playlists": [self._normalize_playlist(p) for p in user_playlists_complete]
        }
    ...
    def _normalize_track(self, t):
        ...
        if not tid or int(tid) == 0: return {"id": 0}

        return {
            "id": int(tid),
            "title": self._find_val(t, ['SNG_TITLE', 'TITLE', 'title'], "Unknown"),
            "duration": int(self._find_val(t, ['DURATION', 'duration'], 0)),
            "explicit": bool(int(self._find_val(t, ['EXPLICIT_LYRICS', 'explicit_lyrics'], 0))),
            "version": self._find_val(t, ['VERSION', 'title_version'], ""),
            "date_add": self._find_val(t, ['DATE_ADD', 'time_add', 'added_at'], 0),
            "artist": {
                "id": self._find_val(t, ['ART_ID', 'ARTIST_ID']) or self._find_val(t.get('ARTIST',{}), ['ART_ID', 'id']),
                "name": self._find_val(t, ['ART_NAME', 'ARTIST_NAME']) or self._find_val(t.get('ARTIST',{}), ['ART_NAME', 'name'])
            },
            "album": {
                "id": self._find_val(t, ['ALB_ID', 'ALBUM_ID']) or self._find_val(t.get('ALBUM',{}), ['ALB_ID', 'id']),
                "title": self._find_val(t, ['ALB_TITLE', 'ALBUM_TITLE']) or self._find_val(t.get('ALBUM',{}), ['ALB_TITLE', 'title']),
                "cover": self._find_val(t, ['ALB_PICTURE', 'ALBUM_PICTURE']) or self._find_val(t.get('ALBUM',{}), ['ALB_PICTURE', 'cover'])
            }
        }
    ...
    def _normalize_album(self, a):
        return {
            ...
            "release_date": self._find_val(a, ['PHYSICAL_RELEASE_DATE', 'release_date']),
            "cover": self._find_val(a, ['ALB_PICTURE', 'cover', 'cover_medium']),
            "artist": {
    ...
    def _normalize_playlist(self, p):
        ...
        clean_tracks = [self._normalize_track(t) for t in raw_tracks if self._normalize_track(t)['id'] != 0]
        return {
            "id": self._find_val(p, ['PLAYLIST_ID', 'id']),
            "title": self._find_val(p, ['TITLE', 'title']),
            "creation_date": self._find_val(p, ['DATE_ADD', 'creation_date'], 0),
            "cover": self._find_val(p, ['PLAYLIST_PICTURE', 'picture', 'cover']),
            "tracks": clean_tracks
        }

# ==========================================
# 3. OUTPUT: MONOCHROME JSON
# ==========================================
...
if __name__ == "__main__":
    src = DeezerGatewaySource()
    fmt = MonochromeJsonOutput()
    try:
        src.authenticate()
        data = src.fetch_data()
        fmt.save(data, "monochrome_export.json")
    except Exception as e:
        print(f"\nERROR: {e}")
        import traceback
        traceback.print_exc()